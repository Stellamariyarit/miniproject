...
def edit_profile(request):
    donor = Donor.objects.get(user=request.user)  # Fetch the donor object related to the logged-in user

    if request.method == 'POST':
        form = DonorForm(request.POST, request.FILES, instance=donor)  # Include request.FILES
        if form.is_valid():
            form.save()  # Update the donor instance with new data
            return redirect('donation_statistics')  # Redirect to the donation success page
    else:
        form = DonorForm(instance=donor)  # Initialize form with existing donor data

    return render(request, 'donor_login.html', {'form': form, 'donor': donor})
